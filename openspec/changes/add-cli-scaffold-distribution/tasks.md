...
- [x] 5.2 Tag and push `v0.1.0`; confirm `release.yml` publishes a GitHub Release containing exactly `crenein-agent_0.1.0_linux_amd64.tar.gz`, `crenein-agent_0.1.0_linux_arm64.tar.gz`, and `checksums.txt`; confirm a non-tag push to `main` creates no release. _(release run 27450078342 success; Release v0.1.0 con exactamente los 3 assets, no draft/prerelease. Merge a main disparó solo `ci`, ningún run de release — cláusula no-tag confirmada.)_
